...
# AV.ONE Drive State LED Bridge (ESP32 ASSI)
# File: drive_state_led.py

# Purpose:
#   - Subscribe to AV.ONE drive state and fault status topics.
#   - Send a compact "STATE <n> FAULT <0/1>" command over serial to an ESP32,
#     which drives the ASSI style indicator light (color and pattern based on state).

# What it does:
#   - Subscribes to:
#       /av1/avone_state/drive_state          (std_msgs/Int32)
#       /av1/fault_latched/fault_latched      (std_msgs/Int32)
#   - Opens a serial connection to an ESP32 (fixed port or auto-detected).
#   - Periodically sends the latest state and fault status to the ESP32.
#   - Includes a topic watchdog: if either topic stops updating for longer than
#     topic_timeout_sec, it sends a single FAULT HOLD command then pauses updates
#     until messages resume.
...
#     Example:
#       "STATE 3 FAULT 0\n"
#   - The ESP32 firmware is expected to parse this line and set the LED output.

# Notes:
#   - This node is deliberately simple and robust:
#       * It only sends commands while topics are "alive".
#       * It can auto-detect common USB serial adapters if enabled.
#       * It prints lines received back from the ESP32 for debugging.
#   - The watchdog is important for safety signaling:
#       if ROS stops publishing, the ASSI should fall back to a fault indication.
...
class ESP32LedController(Node):
        ...
        if serial_port:
            try:
                self.serial_conn = serial.Serial(
                    serial_port, baud_rate, timeout=1, write_timeout=1
                )
                time.sleep(2.5)
                self.serial_conn.reset_input_buffer()
                self.serial_conn.reset_output_buffer()
                self.get_logger().info(
                    f"Connected to ESP32 on {serial_port} at {baud_rate} baud"
                )
    ...
    def send_update(self):
        ...
        state_changed = (
            self.current_drive_state != self.last_sent_state
            or fault_val != self.last_sent_fault
        )
