#!/usr/bin/env python3
import math
from typing import Optional

import rclpy
from rclpy.node import Node

from sensor_msgs.msg import NavSatFix
from geometry_msgs.msg import (
    QuaternionStamped, Quaternion, TransformStamped,
    TwistWithCovarianceStamped
)
from nav_msgs.msg import Odometry
from std_msgs.msg import Header
from tf2_ros import TransformBroadcaster
from tf_transformations import quaternion_from_euler, quaternion_multiply

# -------------------- Constants --------------------
WGS84_A = 6378137.0
WGS84_E2 = 6.69437999014e-3


# -------------------- Conversion Helpers --------------------
def lla_to_ecef(lat_rad, lon_rad, alt):
    sin_lat = math.sin(lat_rad)
    cos_lat = math.cos(lat_rad)
    sin_lon = math.sin(lon_rad)
    cos_lon = math.cos(lon_rad)
    N = WGS84_A / math.sqrt(1.0 - WGS84_E2 * sin_lat * sin_lat)
    x = (N + alt) * cos_lat * cos_lon
    y = (N + alt) * cos_lat * sin_lon
    z = (N * (1.0 - WGS84_E2) + alt) * sin_lat
    return x, y, z


def ecef_to_enu(x, y, z, lat0_rad, lon0_rad, x0, y0, z0):
    dx = x - x0
    dy = y - y0
    dz = z - z0
    sin_lat0 = math.sin(lat0_rad)
    cos_lat0 = math.cos(lat0_rad)
    sin_lon0 = math.sin(lon0_rad)
    cos_lon0 = math.cos(lon0_rad)
    e = -sin_lon0 * dx + cos_lon0 * dy
    n = -sin_lat0 * cos_lon0 * dx - sin_lat0 * sin_lon0 * dy + cos_lat0 * dz
    u =  cos_lat0 * cos_lon0 * dx + cos_lat0 * sin_lon0 * dy + sin_lat0 * dz
    return e, n, u


def is_nan_quaternion(q: Quaternion) -> bool:
    return (
        math.isnan(q.x) or math.isnan(q.y) or
        math.isnan(q.z) or math.isnan(q.w)
    )


def rotate_quaternion_z(q_in: Quaternion, yaw_offset_deg: float) -> Quaternion:
    yaw_offset_rad = math.radians(yaw_offset_deg)
    q_offset = quaternion_from_euler(0.0, 0.0, yaw_offset_rad)
    q_in_list = [q_in.x, q_in.y, q_in.z, q_in.w]
    q_out = quaternion_multiply(q_offset, q_in_list)
    return Quaternion(x=q_out[0], y=q_out[1], z=q_out[2], w=q_out[3])


def yaw_to_quaternion(yaw_rad: float) -> Quaternion:
    q = quaternion_from_euler(0.0, 0.0, yaw_rad)
    return Quaternion(x=q[0], y=q[1], z=q[2], w=q[3])


# -------------------- Main Node --------------------
class FixToOdomWithHeading(Node):
    def __init__(self):
        super().__init__('fix_to_odom_with_heading')

        # Frames
        self.declare_parameter('world_frame', 'odom')
        self.declare_parameter('base_frame', 'gps')
        self.world_frame = self.get_parameter('world_frame').value
        self.base_frame = self.get_parameter('base_frame').value

        # Z behaviour
        self.declare_parameter('z_mode', 'initial')  # "initial" or "zero"
        self.z_mode = str(self.get_parameter('z_mode').value).lower()
        self.z_plane: Optional[float] = None

        # Heading source selection
        self.declare_parameter('heading_source', 'position')
         # Options:
        #   "heading"  = use /heading only
        #   "position" = derive from motion only (preferring /vel if available)
        #   "auto"     = use /heading if valid, else fallback to motion (prefers /vel)
        self.heading_source = self.get_parameter('heading_source').value.lower()

        # Heading correction
        self.declare_parameter('yaw_offset_deg', -90.0)
        self.yaw_offset_deg = float(self.get_parameter('yaw_offset_deg').value)

        # Velocity usage
        self.declare_parameter('use_vel', True)
        self.declare_parameter('vel_topic', '/vel')
        self.declare_parameter('vel_is_ned', False)
        self.declare_parameter('vel_heading_min_speed', 0.2)

        self.use_vel = bool(self.get_parameter('use_vel').value)
        self.vel_topic = str(self.get_parameter('vel_topic').value)
        self.vel_is_ned = bool(self.get_parameter('vel_is_ned').value)
        self.vel_heading_min_speed = float(self.get_parameter('vel_heading_min_speed').value)

        # State
        self.origin_set = False
        self.lat0_rad = None
        self.lon0_rad = None
        self.x0 = self.y0 = self.z0 = None
        self.z_locked_value: Optional[float] = None

        self.latest_q: Optional[Quaternion] = None
        self.prev_e = None
        self.prev_n = None
        self.prev_stamp: Optional[float] = None

        # Latest velocities (ENU)
        self.have_vel = False
        self.vel_e = 0.0
        self.vel_n = 0.0
        self.vel_u = 0.0
        self.vel_cov = [0.0] * 36

        # IO
        if self.heading_source in ('heading', 'auto'):
            self.heading_sub = self.create_subscription(QuaternionStamped, '/heading', self.on_heading, 10)

        if self.use_vel:
            self.vel_sub = self.create_subscription(
                TwistWithCovarianceStamped, self.vel_topic, self.on_vel, 10
            )

        self.fix_sub = self.create_subscription(NavSatFix, '/fix', self.on_fix, 10)
        self.odom_pub = self.create_publisher(Odometry, '/odometry/gps', 10)
        self.tf_broadcaster = TransformBroadcaster(self)

        self.get_logger().info(
            f'Publishing /odometry/gps + TF {self.world_frame}->{self.base_frame} | '
            f'z_mode={self.z_mode}, heading_source={self.heading_source}, '
            f'use_vel={self.use_vel} ({self.vel_topic}, vel_is_ned={self.vel_is_ned})'
        )

    # -------------------- Callbacks --------------------
    def on_heading(self, msg: QuaternionStamped):
        if is_nan_quaternion(msg.quaternion):
            self.get_logger().warn("Received NaN heading quaternion — ignoring /heading")
            return

        corrected_q = rotate_quaternion_z(msg.quaternion, self.yaw_offset_deg)
        corrected_q.z *= -1.0
        self.latest_q = corrected_q

    def on_vel(self, msg: TwistWithCovarianceStamped):
        vx = msg.twist.twist.linear.x
        vy = msg.twist.twist.linear.y
        vz = msg.twist.twist.linear.z

        if self.vel_is_ned:
            e, n, u = vy, vx, -vz
        else:
            e, n, u = vx, vy, vz

        self.vel_e, self.vel_n, self.vel_u = e, n, u
        self.have_vel = True

        cov_in = msg.twist.covariance
        if cov_in and len(cov_in) == 36:
            self.vel_cov = list(cov_in)

    def on_fix(self, msg: NavSatFix):
        if msg.status.status < 0 or math.isnan(msg.latitude) or math.isnan(msg.longitude):
            return

        lat_rad = math.radians(msg.latitude)
        lon_rad = math.radians(msg.longitude)
        alt = msg.altitude
        x, y, z = lla_to_ecef(lat_rad, lon_rad, alt)

        if not self.origin_set:
            self.lat0_rad = lat_rad
            self.lon0_rad = lon_rad
            self.x0, self.y0, self.z0 = x, y, z
            self.origin_set = True
            self.get_logger().info(
                f'Origin set at lat={msg.latitude:.8f}, lon={msg.longitude:.8f}, alt={alt:.2f}'
            )

        e, n, u = ecef_to_enu(x, y, z, self.lat0_rad, self.lon0_rad, self.x0, self.y0, self.z0)

        if self.z_mode == 'zero':
            z_locked = 0.0
        else:
            if self.z_locked_value is None:
                self.z_locked_value = u
                self.get_logger().info(f'Z plane set to {self.z_locked_value:.3f} m')
            z_locked = self.z_locked_value

        # Heading source
        use_heading_topic = self.heading_source in ('heading', 'auto')
        use_motion_heading = self.heading_source in ('position', 'auto')

        heading_valid = self.latest_q is not None and not is_nan_quaternion(self.latest_q)
        used_vel_for_heading = False

        if use_motion_heading and (not use_heading_topic or not heading_valid):
            if self.have_vel:
                speed = math.hypot(self.vel_e, self.vel_n)
                if speed >= self.vel_heading_min_speed:
                    yaw_rad = math.atan2(self.vel_n, self.vel_e)
                    self.latest_q = yaw_to_quaternion(yaw_rad)
                    used_vel_for_heading = True

            if not used_vel_for_heading:
                if self.prev_e is not None and self.prev_n is not None:
                    de = e - self.prev_e
                    dn = n - self.prev_n
                    if abs(de) > 1e-3 or abs(dn) > 1e-3:
                        yaw_rad = math.atan2(dn, de)
                        self.latest_q = yaw_to_quaternion(yaw_rad)

        # Velocity estimation
        twist_e = twist_n = twist_u = 0.0
        twist_cov = [0.0] * 36

        curr_time = msg.header.stamp.sec + msg.header.stamp.nanosec * 1e-9
        dt = None
        if self.prev_stamp is not None:
            dt = max(1e-6, curr_time - self.prev_stamp)

        if self.have_vel:
            twist_e, twist_n, twist_u = self.vel_e, self.vel_n, self.vel_u
            twist_cov = self.vel_cov[:]
        elif dt is not None and self.prev_e is not None and self.prev_n is not None:
            twist_e = (e - self.prev_e) / dt
            twist_n = (n - self.prev_n) / dt
            twist_u = 0.0
            twist_cov[0] = twist_cov[7] = 0.5
            twist_cov[14] = 1e3
        else:
            twist_cov[0] = twist_cov[7] = 1e3
            twist_cov[14] = 1e6

        # ---------------- Publish Odom ----------------
        odom = Odometry()
        odom.header = Header()
        odom.header.stamp = msg.header.stamp
        odom.header.frame_id = self.world_frame
        odom.child_frame_id = self.base_frame

        odom.pose.pose.position.x = e
        odom.pose.pose.position.y = n
        odom.pose.pose.position.z = z_locked

        if self.latest_q is not None and not is_nan_quaternion(self.latest_q):
            odom.pose.pose.orientation = self.latest_q
        else:
            odom.pose.pose.orientation = Quaternion(x=0.0, y=0.0, z=0.0, w=1.0)

        cov = [0.0] * 36
        if msg.position_covariance_type in (1, 2, 3) and len(msg.position_covariance) == 9:
            cov_xy = msg.position_covariance[0]
            cov[0] = cov_xy
            cov[7] = cov_xy
        cov[14] = 0.01
        odom.pose.covariance = cov

        # Rotate ENU vel → body frame
        if self.latest_q is not None and not is_nan_quaternion(self.latest_q):
            q = self.latest_q
            siny_cosp = 2.0 * (q.w * q.z + q.x * q.y)
            cosy_cosp = 1.0 - 2.0 * (q.y * q.y + q.z * q.z)
            yaw = math.atan2(siny_cosp, cosy_cosp)
        else:
            yaw = 0.0

        v_fwd =  math.cos(yaw) * twist_e + math.sin(yaw) * twist_n
        v_lat = -math.sin(yaw) * twist_e + math.cos(yaw) * twist_n

        odom.twist.twist.linear.x = v_fwd
        odom.twist.twist.linear.y = v_lat
        odom.twist.twist.linear.z = twist_u
        odom.twist.covariance = twist_cov

        self.odom_pub.publish(odom)

        # TF
        t = TransformStamped()
        t.header.stamp = msg.header.stamp
        t.header.frame_id = self.world_frame
        t.child_frame_id = self.base_frame
        t.transform.translation.x = e
        t.transform.translation.y = n
        t.transform.translation.z = z_locked
        t.transform.rotation = odom.pose.pose.orientation
        self.tf_broadcaster.sendTransform(t)

        # Update state
        self.prev_e, self.prev_n = e, n
        self.prev_stamp = curr_time


# -------------------- Main --------------------
def main():
    rclpy.init()
    node = FixToOdomWithHeading()
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()


if __name__ == '__main__':
    main()
