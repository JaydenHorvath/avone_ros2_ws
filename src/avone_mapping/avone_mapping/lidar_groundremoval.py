...
# avone_mapping/lidar_groundremoval.py (PointCloud2 ground + range + sector filtering)
# ----------------------------------------------------------------------
# This node filters a raw LiDAR PointCloud2 stream to remove:
#   1) Points outside a radius window (r_min .. r_max)
#   2) "Ground" points below an adaptive height threshold:
#        z >= z_min + k_lin * r + ring_bias[ring]
#      where r is planar distance from the LiDAR, and ring_bias optionally
#      compensates per-laser vertical offsets / mounting tilt / per-ring noise.
#   3) (Optional) Points inside an angular sector (or keep only that sector).
#      The sector can be:
#        - manual: fixed angle_center_deg in the LiDAR frame
#        - base: auto-aligned to the vehicle forward direction using TF so the
#                filter stays correct even if the LiDAR is rotated when mounted.

# Use-case on AV.ONE:
#   - Strip ground returns so cone clustering is more stable
#   - Strip points from the vehicle itself (wheels/legs) using the angle sector
#   - Keep only the "working range" so downstream nodes don't process junk

# Topics:
#   Subscribes:  input_topic  (default /quanergy/points)
#   Publishes:   output_topic (default /points_no_ground)

# Key params to tune:
#   - z_min, k_lin: sets the ground cutoff curve vs range
#   - ring_bias: ring-specific extra z to reject ground per ring
#   - r_min/r_max: processing range window
#   - angle_filter_enabled, angle_half_width_deg, angle_keep, angle_mode: sector behaviour

# Notes / gotchas:
#   - PointCloud2 layout differs between drivers. approx_layout=True assumes x/y/z
#     are float32 at offsets 0/4/8. If your driver differs, set approx_layout=False.
#   - Sector filtering uses atan2(y,x) in the LiDAR frame. If your LiDAR frame axes are
#     different from REP-103 expectations, angle_mode='base' helps because it uses TF.
# """
...
class GroundRemover(Node):
    def __init__(self):
        ...
        # Base filters
        self.declare_parameter("z_min", -0.5)  # base z threshold at r=0 (m)
        ...
        self.declare_parameter(
            "ring_bias", [0.00, 0.00, 0.01, 0.01, 0.02, 0.02, 0.03, 0.03]
        )

        # Angular sector filter
        # angle_mode: 'manual' (use angle_center_deg) or 'base' (use base_frame +X via TF)
        self.declare_parameter("angle_filter_enabled", False)
        ...
        self.declare_parameter(
            "angle_half_width_deg", 90.0
        )  # widen to catch legs while pulling
        ...
        self.get_logger().info(
            f"Ground+range+angle on {in_topic} → {out_topic} | angle_mode={self.get_parameter('angle_mode').value}"
        )
    ...
    def cb(self, msg: PointCloud2):
        try:
            ...
            buf = np.frombuffer(msg.data, dtype=np.uint8, count=n * step).reshape(
                (-1, step)
            )
            ...
            if bool(self.get_parameter("angle_filter_enabled").value):
                ...
                half = np.deg2rad(
                    float(self.get_parameter("angle_half_width_deg").value)
                )
                ...
                if mode == "base":
                    ...
                    center = (
                        self._forward_angle_base_in_lidar(
                            lidar_frame, base_frame, stamp, fallback_deg=0.0
                        )
                        + np.pi
                    )
                else:
                    ...
                    center = np.deg2rad(
                        float(self.get_parameter("angle_center_deg").value) + 180.0
                    )
    ...
    # ---------- helpers ----------
    def _forward_angle_base_in_lidar(
        self, lidar_frame: str, base_frame: str, stamp, fallback_deg: float
    ) -> float:
        """
        ...
        try:
            ...
            can = self.tf_buffer.can_transform(
                lidar_frame, base_frame, stamp, timeout=Duration(seconds=0.2)
            )
        except Exception:
            can = False
        try:
            if can:
                ...
                self.get_logger().warn(
                    f"Angle: TF at stamp not ready; using latest {base_frame}->{lidar_frame}"
                )
            R = _quat_to_rot(
            ...
            self.get_logger().warn(
                f"Angle: TF lookup failed ({e}); using fallback {fallback_deg} deg"
            )
    ...
    def _xyz_layout_ok(msg: PointCloud2) -> bool:
        try:
            ...
            return (
                f["x"].datatype == PointField.FLOAT32
                and f["x"].offset == 0
                and f["y"].datatype == PointField.FLOAT32
                and f["y"].offset == 4
                and f["z"].datatype == PointField.FLOAT32
                and f["z"].offset == 8
            )
        except KeyError:
            return False
